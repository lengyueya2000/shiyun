...
一款 Android 原生古诗词阅读应用,以宣纸、墨、朱红构成的古典视觉呈现中华诗词。数据来自 [chinese-poetry](https://github.com/chinese-poetry/chinese-poetry) 开源数据集,经本地管线处理为约 2589 首精选诗词,覆盖先秦/魏晋/唐/宋/元/明清(汉暂未收录,随数据源补充),首启自动导入 Room 数据库。

## 功能

- **首页 · 今日一诗**:每日按日期稳定推荐一首精选诗词,支持一键打卡并累计连续打卡天数。
- **诗词库**:支持关键词搜索(分词匹配)、按朝代/体裁筛选,结果为空时展示空态。
- **收藏与详情**:诗词详情页含译文/注释/赏析区块(数据缺失时不显示),可收藏/取消收藏,收藏页集中管理。
- **背诗闯关**:按朝代分关的问答玩法,含填字、上下句衔接、作者归属三种题型,按正确率给星并解锁下一关。
